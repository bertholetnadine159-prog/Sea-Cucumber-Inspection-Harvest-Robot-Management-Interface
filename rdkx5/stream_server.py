    ...
    def _handle_command(self, command: str, params: dict[str, Any]) -> dict[str, Any]:
        try:
            ...
            if command == "esc_calibrate":
                ...
                result = self.pixhawk.esc_calibrate(channels)
                return {"type": "ack", "command": command, "success": True, "result": result}

            if command == "calibrate_one_way":
                channels = params.get("channels")
                if isinstance(channels, list):
                    channels = [int(c) for c in channels]
                result = self.pixhawk.calibrate_one_way(channels)
                return {"type": "ack", "command": command, "success": True, "result": result}
