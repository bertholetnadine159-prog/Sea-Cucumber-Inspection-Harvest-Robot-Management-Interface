...
def request_context(master: object, system: int, component: int, mavutil: object) -> None:
    """请求遥测流与固件版本消息，确保 STATUSTEXT/SYS_STATUS 有稳定下发上下文。"""
    try:
...
def main() -> int:
    ...
    if args.arm and not args.i_confirm_propellers_removed:
    ...
    for name in ("ARMING_CHECK", "BATT_MONITOR", "RC1_MIN", "RC1_MAX", "RC1_TRIM", "BRD_VBUS_MIN"):
